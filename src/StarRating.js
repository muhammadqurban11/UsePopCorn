...
export default function StarRating({ maxRating = 5, color = '#fcc419', size = 48, className = "", message = [], onSetRating, defaultRating = 0, }) {
        ...
        <div style={ContainerStyle} className={className}>
            <div style={ContainerStar}>
                {Array.from({ length: maxRating }, (_, i) => (<Star key={i} onrate={() => handleRating(i + 1)}
                    full={tempRating ? tempRating >= i + 1 : rating >= i + 1} onHoverIn={() => setTempRating(i + 1)}
